...
class SimpleClassifier(torch.nn.Module):
    def __init__(self, nb_filters, nb_layers, nb_blocks, dense, nb_class=1000, input_size=256):
        """
        ...
        if len(self.dense) > 1:
            for d in range(1, len(self.dense)):
                ...
                layers.append(torch.nn.BatchNorm1d(self.dense[d]))
                layers.append(torch.nn.ReLU())

        #Classes' evidence layer
        layers.append(torch.nn.Linear(self.dense[-1], self.nb_class))
